...
<template>
    <yd-layout>
        <yd-navbar slot="navbar" :title="title"> 
            ...
            <yd-icon name="more" slot="right" @click.native="poptipShow" v-if="functionList.length > 0"></yd-icon>
        </yd-navbar>
        <!-- 更多功能 -->
            ...
            <div class="more-function" v-show="poptipVisible">
                <yd-cell-group>
                    <yd-cell-item v-for="fun in functionList" v-bind:key="fun.key" type = 'div'  @click.native="functionClick(fun.key)">
                        <yd-icon slot="icon" :name="fun.icon" size=".42rem"></yd-icon>
...
export default {
    ...
    created(){
        // 设置右上角更多功能项
        util.bus.$on('main-more-action', (actions) => { //Hub接收事件
            console.log("----------------功能设置-----------------");
